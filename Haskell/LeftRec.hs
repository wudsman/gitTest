...
insert_modified_prods :: String -> [[Part]] -> [[Part]]

insert_modified_prods name (x:xs) = 
    ([x] : (x : [NT name])) ++ insert_modified_prods name xs 
...
eliminate_direct_left_recursion (p @ (P name parts)) used_names = (p,[],used_names)
      --let lr_parts = filter (\x -> is_left_recursive (P name x)) parts in
      --let non_lr_parts = remove_all lr_parts parts in
      --let name' = new_name name used_names in
      --let new_name_parts = insert_modified_prods
